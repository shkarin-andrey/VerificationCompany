...
const CreateCompany: NextPage<{ company: ListCompanyInterface }> = ({
  company,
}) => {
  const { finance, companyInfo, title, INN } = company;
  ...
  const formik = useFormik({
    initialValues: {
      ...
      userProfession: company.user.userProfession || "",
      expense: finance.expense || "",
      income: finance.income || "",
      NGEE: companyInfo.NGEE.join(", ") || "",
      OKVED: companyInfo.OKVED || "",
      actualAddress: companyInfo.address.actualAddress || "",
      legalAddress: companyInfo.address.legalAddress || "",
      streetAddress: companyInfo.address.streetAddress || "",
      fullName: companyInfo.fullName || "",
      littleName: companyInfo.littleName || "",
      phones: companyInfo.phones || "",
      site: companyInfo.site || "",
    },
    onSubmit: async (values) => {
      const id = edit ? edit[0] : "";
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4">
      <div className="flex gap-2 items-center mt-5 mb-5">
        ...
        <h1 className="font-bold text-2xl">Редактирование компании</h1>
      </div>
